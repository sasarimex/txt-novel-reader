...
async function run() {
  ...
  try {
    ...
    assert.equal(typeof imported.bookHash, 'string');
    assert.equal(imported.bookHash.length, 64);
    ...
    assert.equal(records[0].tocParseVersion, CURRENT_TOC_PARSE_VERSION);
    ...
    await assert.rejects(
      ...
      (error) => error.code === 'DUPLICATE_BOOK_HASH',
    );
